...
contract Vault is IVault, ERC20, DefaultAccessControl, ReentrancyGuard {
    ...
    function pendingWithdrawers(
        ...
        if (offset >= count || limit == 0) return result; // non initialized
        count -= offset;
        if (count > limit) count = limit;
        result = new address[](count);
        for (uint256 i = 0; i < count; i++) {
            result[i] = withdrawers_.at(offset + i); // get array position
        }
        return result;
    }
    ...
    function _calculateTvl( // Total Value Locked
        ...
        for (uint256 i = 0; i < tvl_.length; i++) {
            ...
            for (uint256 j = 0; j < tokens.length; j++) {
                if (token != tokens[j]) continue; // find the token
                (data.isDebt ? negativeAmounts : amounts)[j] += amount; // 2 cases
                break;
            }
        }
        for (uint256 i = 0; i < tokens.length; i++) {
            if (amounts[i] < negativeAmounts[i]) revert InvalidState(); 
            // all negativeAmounts should be less than amounts
    ...
    function addToken(address token) external nonReentrant {
        ...
        // are these 4 checks enough? whitelist needed?
        _isUnderlyingToken[token] = true;
        address[] storage tokens = _underlyingTokens;
        tokens.push(token);
        uint256 n = tokens.length;
        uint256 index = 0;
        for (uint256 i = 1; i < n; i++) { // too many gas consumption!
            address token_ = tokens[n - 1 - i];
            if (token_ < token) { // sort the tokens; need sorted?
                index = n - i;
                break;
            }
            tokens[n - i] = token_;
    ...
    function removeToken(address token) external nonReentrant {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            if (tokens[i] == token) {
                if (amounts[i] != 0) revert NonZeroValue(); // should be zero
                index = i;
                break;
            }
        }
        ...
        while (index + 1 < tokens.length) {
            _underlyingTokens[index] = tokens[index + 1]; // move; gas consumption!
            index++;
        }
    ...
    function delegateCall(
        ...
        if (!configurator.isDelegateModuleApproved(to)) revert Forbidden();
        IValidator validator = IValidator(configurator.validator());
        validator.validate( // shoule be checked later
            msg.sender,
            address(this),
            abi.encodeWithSelector(msg.sig, to, data)
        );
    ...
    function _processLpAmount(
        ...
        if (lpAmount + totalSupply > configurator.maximalTotalSupply()) // potential ADD overflow
        ...
        returns (uint256[] memory actualAmounts)
    {
        ...
        WithdrawalRequest memory request = _withdrawalRequest[sender]; // is this defined?
        // so it supposes that USER should call withdrawalRequest() before
        // and then call emergencyWithdraw() to withdraw the funds
        // is it reasonable?
        ...
        checkDeadline(requestDeadline)
    {
        ...
        if (_pendingWithdrawers.contains(sender)) {
            ...
            _cancelWithdrawalRequest(sender); // cancel previous request
        }
        ...
        if (tokens.length != minAmounts.length) revert InvalidLength(); // ! an important check !
    ...
    function analyzeRequest(
        ...
        uint256 value = FullMath.mulDiv(lpAmount, s.totalValue, s.totalSupply); // if ZERO?
        value = FullMath.mulDiv(value, D9 - s.feeD9, D9); // if s.feeD9 > D9?
        ...
        returns (ProcessWithdrawalsStack memory s)
    {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            s.ratiosX96Value += FullMath.mulDiv(s.ratiosX96[i], priceX96, Q96); // should RoundingUp
