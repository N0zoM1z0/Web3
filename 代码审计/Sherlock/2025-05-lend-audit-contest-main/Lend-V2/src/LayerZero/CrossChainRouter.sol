...
contract CrossChainRouter is OApp, ExponentialNoError {
    ...
    function borrowCrossChain(uint256 _amount, address _borrowToken, uint32 _destEid) external payable {
        ...
        require(address(this).balance > 0, "Out of money"); // 非常奇怪的检查。这里检查的是 CrossChainRouter 合约自身的ETH余额是否大于0，而不是检查 msg.value 是否足够支付 LayerZero费用。结合 _payNative 和 _send 中使用 address(this).balance 作为费用，这意味着用户必须先向 CrossChainRouter 合约发送ETH（可能通过 receive()，或者 msg.value 应该被正确使用）来预付 LayerZero 的消息费用。 如果用户没有发送ETH (payable modifier)，或者合约ETH余额不足，这里可能出问题或依赖于预存的ETH。
    ...
    function _validateAndPrepareLiquidation(LendStorage.LiquidationParams memory params) private view {
        ...
        for (uint256 i = 0; i < userCrossChainCollaterals.length;) {
            if (userCrossChainCollaterals[i].srcEid == params.srcEid) { // find
                found = true;
        ...
        uint256 maxLiquidationAmount = lendStorage.getMaxLiquidationRepayAmount( // 这里 getMaxLiquidationRepayAmount 找的是一个dst和src都一样的同链的。。。
            params.borrower,
    ...
    function _updateBorrowPositionForLiquidation( // 然而这个函数被弃用了，没有调用点。。。tmd。。。
...
// msg.sender 不对！！！ 这是个private！！！怎么能是 msg.sender？应该是 params.borrower
        for (uint256 i = 0; i < userBorrows.length;) {
            if (userBorrows[i].srcEid == params.srcEid) {
                ...
                lendStorage.updateCrossChainCollateral(msg.sender, params.borrowedAsset, i, userBorrows[i]); // 再次使用了错误的msg.sender！
                break;
            }
            unchecked {
                ++i;
    ...
    function repayCrossChainBorrowInternal(
        ...
        (uint256 borrowedAmount, uint256 index, LendStorage.Borrow memory borrowPosition) = // borrowedAmount 是带利息的
    ...
    function _updateRepaymentState(
        ...
        LendStorage.Borrow[] memory userCrossChainCollaterals = lendStorage.getCrossChainCollaterals(borrower, _token);
// 这里实现了CoreRouter里面 cross-chain 的更新
...
// 这里要确保 源链A和债务链B 的单位和精度是一致的
    ...
    function _handleBorrowCrossChainRequest(LZPayload memory payload, uint32 srcEid) private {
        ...
        require(payload.collateral >= totalBorrowed, "Insufficient collateral"); // 这个payload.collateral 是之前的源链A的，存在一个时间差风险
    ...
    function _handleValidBorrowRequest(LZPayload memory payload, uint32 srcEid) private {
        ...
        if (found) {
            ...
            LendStorage.Borrow[] memory userBorrows = lendStorage.getCrossChainBorrows(payload.sender, payload.srcToken);
            userBorrows[index].principle = userBorrows[index].principle + payload.amount; // 应该更新principle with interest后再累加！！！
            // uint256 currentValueOldPrincipal = (userBorrows[index].principle * payload.borrowIndex) / userBorrows[index].borrowIndex;
