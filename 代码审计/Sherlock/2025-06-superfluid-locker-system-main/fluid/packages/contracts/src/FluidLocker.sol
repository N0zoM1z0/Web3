...
contract FluidLocker is Initializable, ReentrancyGuard, IFluidLocker {
    ...
    function stake(uint256 amountToStake) external nonReentrant onlyLockerOwner unlockAvailable {
        ...
        stakingUnlocksAt = uint80(block.timestamp) + _STAKING_COOLDOWN_PERIOD; // 有最低质押期限 - 冷却期
    ...
    function unstake(uint256 amountToUnstake) external nonReentrant onlyLockerOwner unlockAvailable {
        if (block.timestamp < stakingUnlocksAt) { // 冷却期检查
        ...
        FLUID.disconnectPool(STAKER_DISTRIBUTION_POOL); // 这里是无条件执行！！！如果用户只是部分赎回，那就不应该完全disconnect！！！
// 后果：用户无法获得剩余的质押token
        emit FluidUnstaked();
    }
    ...
    function provideLiquidity(uint256 supAmount) external payable nonReentrant onlyLockerOwner {
        ...
        _pump(weth, ethAmount * BP_PUMP_RATIO / BP_DENOMINATOR); // 内部uniswapv3缺乏滑点保护
...
// 没有else！！！  在处理未满足6个月免税条件的提款时，没有对撤回的SUP代币进行任何处理，会导致用户资金被永久锁定在Locker合约中 
