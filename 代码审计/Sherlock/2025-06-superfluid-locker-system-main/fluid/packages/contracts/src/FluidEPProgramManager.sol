...
contract FluidEPProgramManager is Initializable, OwnableUpgradeable, EPProgramManager, IUserDefinedMacro {
    ...
    function cancelProgram(uint256 programId) external onlyOwner {
        ...
        _updateFundingFlowRateFromTreasury(
            program.token, -(programDetails.fundingFlowRate + programDetails.subsidyFlowRate) // 与startFunding保持一致
        );

        // Return the initial deposit to the treasury
        // 为什么return initial deposit???不应该return剩余的未分发的吗
        uint256 buffer =
    ...
    function startFunding(uint256 programId, uint256 totalAmount, uint32 programDuration) external onlyOwner {
        ...
        int96 subsidyFlowRate = int256(subsidyAmount / programDuration).toInt96(); // ZERO division and potential Overflow
        ...
        // Update the funding flow rate from the treasury
        _updateFundingFlowRateFromTreasury(program.token, fundingFlowRate + subsidyFlowRate); // 这里的 fundingFlowRate + subsidyFlowRate 与前面的 cancelProgram() 中的 fundingFlowRate + subsidyFlowRate 保持一致
    ...
    { // 这个内部函数的作用是调整从金库 (fluidTreasury) 到本合约的总流入流率
    // 维持平衡：从金库流入的总流率 = 所有向外流出的流率之和
