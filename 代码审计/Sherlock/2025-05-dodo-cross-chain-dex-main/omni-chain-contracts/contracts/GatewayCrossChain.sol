...
contract GatewayCrossChain is UniversalContract, Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function _swapAndSendERC20Tokens(
        ...
        uint amountInMax = (amountsQuote[0]) + (slippage * amountsQuote[0]) / 1000; // 考虑滑点
        IZRC20(targetZRC20).approve(UniswapRouter, amountInMax); // 授权max

        // Swap TargetZRC20 to gasZRC20
        uint[] memory amounts = IUniswapV2Router01(UniswapRouter)
            .swapTokensForExactTokens(
                gasFee, // Amount of gas token required amountOut
                amountInMax, // amountInMax
                getPathForTokens(targetZRC20, gasZRC20), // path[0] = targetZRC20, path[1] = gasZRC20 address[] calldata path
                address(this),
                block.timestamp + MAX_DEADLINE // uint256 deadline
        );

        require(IZRC20(gasZRC20).balanceOf(address(this)) >= gasFee, "INSUFFICIENT_GAS_FOR_WITHDRAW");
        require(targetAmount - amountInMax > 0, "INSUFFICIENT_AMOUNT_FOR_WITHDRAW"); // ? 不应该用Max check啊
// 应该是: require(targetAmount - amount[0] > 0, "INSUFFICIENT_AMOUNT_FOR_WITHDRAW");
    ...
    function _handleEvmOrSolanaWithdraw(
        ...
        if (decoded.targetZRC20 == gasZRC20) { // 场景A：目标代币本身就是Gas代币 
            if (gasFee >= outputAmount) revert NotEnoughToPayGasFee();
            IZRC20(decoded.targetZRC20).approve(address(gateway), outputAmount + gasFee);
            ...
            bytes memory encoded = (decoded.dstChainId == SOLANA_EDDY)
                ...
                : data; // 这里需要再次审计一下 AccountEncoder的两个是否匹配
    ...
    ) external onlyGateway { // gateway转发的是端消息，不能保证message不是malicious的 认证消息的传递者，不等于认证消息的内容
    ...
    function onRevert(RevertContext calldata context) external onlyGateway {
        ...
        if(context.revertMessage.length == 52) { // 是否过于武断？ 无论是用户钱包地址（EOA）还是智能合约地址，它们的长度都是20字节
// 如果原始交易的接收方是一个20字节的合约地址，这部分代码会错误地执行直接转账，而不是走下面 else 分支的机器人/手动认领流程。
// 违反了Q&A的退款流程描述
// 最严重的后果是可能导致用户退款的资产被永久锁定在接收方合约中，造成直接的资金损失
    ...
    function claimRefund(bytes32 externalId) external {
        ...
        if(refundInfo.walletAddress.length == 20) { // 如果是EVM地址
            receiver = address(uint160(bytes20(refundInfo.walletAddress)));
        }
        // 但如果不是EVM，如果是btc/sol 那么下面的 msg.sender == receiver 完全会被绕过！！！
        require(bots[msg.sender] || msg.sender == receiver, "INVALID_CALLER"); // 任何人都可以领取非EVM地址的退款！！！
