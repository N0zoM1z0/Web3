...
contract EggHuntGame is Ownable {
    ...
    function searchForEgg() external {
        ...
        uint256 random = uint256(
            keccak256(abi.encodePacked(block.timestamp, block.prevrandao, msg.sender, eggCounter)) // !!! maybe
        ) % 100;

        if (random < eggFindThreshold) {
            eggCounter++; // overflow -> revert
    ...
    function depositEggToVault(uint256 tokenId) external { // deposit egg NFT to the Egg Vault FIRST
