...
pub fn create_ata_instruction(wallet: Pubkey, mint: Pubkey) -> (Instruction, Pubkey) {
    let ata = spl_associated_token_account::get_associated_token_address(&wallet, &mint);
    (spl_associated_token_account::instruction::create_associated_token_account(&wallet, &wallet, &mint, &spl_token::ID), ata)
}